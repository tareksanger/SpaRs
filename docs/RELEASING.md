...
This run rebuilds and retests the release before publishing its own tested tarballs. It publishes both platform packages before the main package. All three packages use the release version. The source manifest allows public publication. Use `cargo publish-npm` for releases: it uses the workflow or its complete tarball set, including native dependencies and notices. Running `npm publish` directly inside `bindings/node` does not assemble that release set. There are no publication lifecycle hooks.
