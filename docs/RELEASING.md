...
[Release Please](https://github.com/googleapis/release-please) prepares one release PR for the root `spars-nlp` crate. It updates `Cargo.toml`, the root lockfile, the root crate entries in the three dependent lockfiles, the release manifest, and `CHANGELOG.md`. The same release version is applied to the installer and Node Rust manifests, the Node npm manifest, and their lockfile entries. Publication settings stay unchanged. Their changes are included in the shared changelog.
