...
CI regenerates the small, large, and component-order references in the required macOS ARM job. The Linux job uses `tools/verify.py --model-exports-only`: it still verifies byte-identical model exports, fixture checksums, the existing medium-model reference suites, and all native parity tests against frozen expectations, with unchanged tolerances and exact discrete outputs. It omits only the small/large/component-order Python reference regeneration performed by `tools/check_models.py`. A Linux pass alone does not establish that regeneration passed; releases require both platforms.

For standalone export verification use `.venv/bin/python tools/check_models.py --exports-only`. Full regeneration remains the default for both tools. Export hashes are saved to `target/reports/model-exports.json`; full regeneration also writes `target/reports/model-references.json`. The acceptance report records `model_reference_regeneration` so an export-only run cannot be mistaken for full model-reference regeneration. The model checker clears its previous export and reference reports before each run, so skipped or failed comparisons do not leave stale success reports. Inspect the acceptance report and artifacts for the exact CI run.
