...
Ordinary `cargo test` skips model-dependent tests. Acceptance and CI include them explicitly. CI runs the verification script on Linux with `--model-exports-only`, including all native parity tests, formatting, typing, documentation, packaging, and tooling checks. macOS ARM runs small/large/component-order reference regeneration and builds the native installer, Rust consumer, and Node binding, then verifies and processes all three model sizes with interpreters absent from `PATH`. It does not repeat the full acceptance suite; see [validation](VALIDATION.md#model-reference-regeneration-across-platforms). Both jobs upload available reports even on failure. Rust dependency builds are cached separately by job and toolchain for the root and consumer workspaces; cache hits never skip checks. Benchmarks run on Linux. Repository administrators should require both reference jobs (`reference-and-rust (ubuntu-latest)` and `reference-and-rust (macos-latest)`) and `native-installation` before merging; the workflow alone does not configure branch protection.
