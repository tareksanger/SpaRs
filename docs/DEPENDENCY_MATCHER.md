...
This is a typed Rust API, not an importer for arbitrary spaCy matcher JSON. Regex, fuzzy matching, custom extensions, callbacks, span inputs, integer rule IDs, additional lexical flags, and token repetition operators are not exposed yet. Token Matcher and PhraseMatcher remain separate planned features. Pattern JSON uses the Rust types' fields and rejects unknown fields or enum values. Full spaCy matcher compatibility is not claimed.
