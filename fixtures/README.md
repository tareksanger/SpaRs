...
## Additional model and capability references

`model-sm-v1.expected.json` and `model-lg-v1.expected.json` reuse the frozen project-authored MIT inputs in `evaluation-v1.json` (98 documents / 5,568 tokens), with independent outputs from each official 3.8.0 package under spaCy 3.8.14 / Thinc 8.3.13. They were generated before comparing native outputs and are regression evidence for the model expansion. The original medium-model inputs and expectations remain unchanged. Regenerate to new paths with `tools/evaluation_reference.py --model en_core_web_sm INPUT OUTPUT` or `--model en_core_web_lg` using the pinned Python environment.

`model-capabilities-v1.expected.json` contains project-authored MIT cases for small-model contextual token, document, proper-span and empty-span vectors, plus medium-model NER-first execution on multi-sentence and empty texts. `tools/model_reference.py OUTPUT` obtains these outputs from the official components in the declared order and refuses to overwrite a fixture. These cases influenced the optional-vector and configurable-order implementations. `tools/check_models.py` regenerates all three new reference files and compares re-exported model assets during acceptance. Checksums of these additions are recorded separately; no prior fixture checksum is changed.
