...
import provenance
import thinc
from model_catalog import catalog, release
from export_capabilities import Capabilities, validate as validate_capabilities
...
def export(out: Path, model_name: str = 'en_core_web_md') -> None:
    assert spacy.__version__ == '3.8.14' and thinc.__version__ == '8.3.13'
    entry = release(model_name)
    nlp = spacy.load(entry.model)
    capabilities = validate_capabilities(nlp)
    assert nlp.meta['version'] == entry.version
    source_info=provenance.record(entry.model)
    assert digest(entry.wheel) == entry.wheel_sha256
    ...
    def tok2vec(model: Model, prefix: str) -> Tok2VecConfig:
        ...
        static = next((x for x in nodes if x.name == 'static_vectors'), None)
        return {'width':model.get_dim('nO'), 'attrs':attrs,
            ...
            'static':params(static, prefix+'.static') if static is not None else None,
    ...
    manifest: Manifest = {'format_version':entry.format_version,'model':entry.model,'model_version':entry.version,
        ...
        'source':f'https://github.com/explosion/spacy-models/releases/tag/{entry.model}-{entry.version}',
    ...
    modeldir = Path(str(md.distribution(entry.model).locate_file(entry.prefix)))
...
if __name__ == '__main__':
    ...
    if options.all and options.out is not None:
        parser.error('--out requires a single model')
    for entry in catalog() if options.all else [release(options.model)]:
        export(options.out or Path('assets') / f'{entry.model}-{entry.version}', entry.model)
