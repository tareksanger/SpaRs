...
def main():
    ...
    for profile in ('load', 'short', 'long'):
        ...
        with tempfile.TemporaryFile() as output:
            ...
            output.seek(0)
            measurements = json.load(output)
        measurements['peak_process_rss_bytes'] = usage.ru_maxrss * (1 if platform.system() == 'Darwin' else 1024)
        measurements['command'] = cmd
    ...
    report = {'platform':platform.platform(), 'cpu':cpu,'logical_cpus':os.cpu_count(), 'ram_bytes':ram,
